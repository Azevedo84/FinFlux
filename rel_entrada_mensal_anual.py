import sys
from forms.tela_rel_entrada_mensal_anual import *
...
class TelaRelatorioEntradaMensal(QMainWindow, Ui_MainWindow):
        ...
        if not self.processando:
            try:
                ...
                if classifica == "MAIOR VALOR":
                    order_by = "ORDER BY mov.qtde_ent DESC"
                elif classifica == "MENOR VALOR":
                    order_by = "ORDER BY mov.qtde_ent"
        ...
        try:
            ...
            cursor.execute(f"""{texto_padrao}
                        WHERE MONTH(mov.data) = {num_mes}
                          ...
                          AND gr.id IN (1, 14) AND cat.id NOT IN (103, 158, 109, 110) 
                        {order_by};
                    """)
            lista_completa = cursor.fetchall()
    ...
    def total_mensal_grupo(self, num_mes, ano_int, num_grupo, order_by):
        ...
        try:
            ...
            cursor.execute(f"""{texto_padrao}
                        WHERE MONTH(mov.data) = {num_mes}
                          AND YEAR(mov.data) = {ano_int} 
                          AND gr.id IN (1, 14) AND cat.id NOT IN (103, 158, 110, 109) 
                          AND gr.id = {num_grupo} 
                        {order_by};
                    """)
            lista_completa = cursor.fetchall()
    ...
    def adicionar_grafico(self, num_mes, ano_int):
        ...
        try:
            ...
            cursor.execute(f"""
                SELECT gr.descricao, 
                       SUM(mov.qtde_ent) AS total_valor
                FROM movimentacao AS mov
                INNER JOIN saldo_banco AS sald ON mov.id_saldo = sald.id
                INNER JOIN cadastro_usuario AS user ON sald.id_usuario = user.id
                INNER JOIN cadastro_banco AS banc ON sald.id_banco = banc.id
    ...
    def gerar_excel(self):
        ...
        try:
            ...
            if dados_tabela:
                ...
                # Cabeçalho
                headers = ["Data", "Usuário", "Banco", "Grupo", "Categoria", "Valor", "Estabelecimento", "Cidade",
                           "Observação"]
                ws.append(headers)
                ...
                # Adiciona os dados
                for linha_idx, (id_mov, data, banco, categoria, valor, estab, cidade, obs) in enumerate(dados_tabela,
                                                                                                        start=2):
                    cursor = conecta.cursor()
...
if __name__ == '__main__':
    ...
    tela = TelaRelatorioEntradaMensal()
    tela.show()
    qt.exec_()
